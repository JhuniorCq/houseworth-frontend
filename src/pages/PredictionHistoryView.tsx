...
const PredictionHistoryView = () => {
  ...
  const [getPredictionById, { isLoading: predictionByIdIsLoading }] =
    useLazyGetPredictionByIdQuery();
  const [getPredictionsByExcelId, { isLoading: predictionByExcelIdIsLoading }] =
    useLazyGetPredictionsByExcelIdQuery();
  ...
  if (
    ...
    const handlePredictionById = async (id: number) => {
      try {
        const result = await getPredictionById(id).unwrap();

        if (!isApiSuccessResponse(result)) return;

        navigate(`/prediction-results/${result.data?.id}`, {
          state: result.data,
        });
      } catch (error) {
        console.error("Error al obtener la predicción por ID: ", error);
        showToast({
          title: "No se pudo obtener los detalles de la predicción.",
          icon: "error",
        });
      }
    };

    const handlePredictionsByExcelId = async ({
      excelId,
      excelName,
    }: {
      excelId: number;
      ...
      try {
        const result = await getPredictionsByExcelId(excelId).unwrap();

        if (!isApiSuccessResponse(result)) return;

        navigate(`/multiple-prediction-results/${excelId}`, {
          state: { predictions: result.data, excelName },
        });
      } catch (error) {
        console.error(
          "Error al obtener las predicciones por Excel ID: ",
          error
        );
        showToast({
          title: "No se pudieron obtener los detalles de las predicciones.",
          icon: "error",
        });
      }
    };

    const goToPredictionDetails = async ({
      prediction,
    }: {
      prediction: PredictionResult;
    }) => {
      if (prediction.excelId) {
        handlePredictionsByExcelId({
          excelId: prediction.excelId,
          excelName: prediction.excelName || "predictions.xlsx",
        });
      } else {
        handlePredictionById(prediction.id);
      }
    };

    return (
      <>
        {(predictionByIdIsLoading || predictionByExcelIdIsLoading) && (
          <div className="bg-white/70 fixed inset-0 z-50 flex justify-center items-center">
            <p className="w-full px-8 text-center font-semibold text-earth-very-strong lg:text-lg">
              Obteniendo detalles ...
            </p>
          </div>
        )}
